...
const argv = yargs(process.argv.slice(2))
    ...
    .options({
        ...
        'dev-mode': {
            description: 'Is dev mode enabled. Allows faster development without connection to Yandex Games Platform',
            default: false,
            type: 'boolean',
        },
    })
...
const S3_PROD_URL = 'http://games.s3.yandex.net';
const S3_DEV_URL = 'http://yastatic.net';
...
app.get(
    '/sdk.js',
    proxy(argv['dev-mode'] ? S3_DEV_URL : S3_PROD_URL, {
        proxyReqPathResolver: (_req) => {
            return argv['dev-mode'] ? '/s3/games-static/sdk-dev/yandex-dev-adapter.js' : '/sdk.js';
        },
    }),
);
...
server.listen(PORT, () => {
    ...
    if (argv['dev-mode']) {
        ...
        console.info('Server is running on', underline(blue(`${localUrl}/`)));

        url = `https://yandex.${argv.tld || 'ru'}/games/app/${
            argv['app-id'] || red('<YOUR_APP_ID>')
        }/?draft=true&game_url=${localUrl}`;
    }

    console.info('You can open your game with', underline(blue(url)));

    if (argv['app-id'] || argv['dev-mode']) {
        open(url);
    }
});
